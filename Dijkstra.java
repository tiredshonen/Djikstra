...
public class Dijkstra {
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy){
      double xCoord = vx - ux;
      xCoord = Math.pow(xCoord, 2);
      
      double yCoord = vy - uy; 
      yCoord = Math.pow(yCoord, 2); 
      
      double sum = xCoord + yCoord; 
  ...
  public void computeAllEuclideanDistances() {
      for (Vertex vertices : getVertices()){
          
          List<Edge> edges = vertices.adjacentEdges; 
          for (Edge outEdge : edges){
              
              double sourceX = (double) outEdge.source.x; 
              double sourceY = (double) outEdge.source.y; 
              double targetX = (double) outEdge.target.x; 
              double targetY = (double) outEdge.target.y; 
              outEdge.distance = computeEuclideanDistance(sourceX, sourceY,
                                                          targetX, targetY);
          }
      }
  }
  ...
  public void doDijkstra(String s) {
      ...
      LinkedList<Vertex> seen = new LinkedList<Vertex>();
      LinkedList<Vertex> pQueue = new LinkedList<Vertex>();
            
      computeAllEuclideanDistances(); 
      
      //Set all vertices to "!known" and "distance = INFINITY"
      //Add all to pQueue for book-keeping
      for (Vertex vertices : getVertices()){
          vertices.distance = Double.POSITIVE_INFINITY; 
          vertices.known = false;
          pQueue.add(vertices); 
      }
      
      //Start at starting vertex; init distance to 0 
      start.distance = 0; 
      
      //While there are unsettled vertices
      while (pQueue.size() != 0){
          //Find the vertex with smallest distance since not an actual pQueue
          Vertex v = pQueue.getFirst(); 
          for (Vertex vertices : pQueue){
              if (vertices.distance < v.distance){
                  v = vertices;
              }
          }
          ...
          //Mark vertex as seen 
          v.known = true; 
          int index = pQueue.indexOf(v);
          seen.add(pQueue.remove(index)); 
      
          //Go through all adjacent edges of the vertex 
          //Update distance of path to vertex according to Djikstra's 
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
      ...
      while (target != source){
          for (Edge e : target.prev.adjacentEdges){
              if (e.target == target){
                  path.addFirst(e);
                  break; 
              }
      ...
      List<Edge> returnPath = (List<Edge>) path;
      return returnPath; 
  }
